...
export class ProjectMessenger {
  ...
  constructor() // peerToken: PeerToken,
    ...
    if (channel) {
      ...
      WEBVIEW_TO_CORE_PASS_THROUGH.forEach((messageType) => {
        channel.on(messageType, async (msg) => {
          console.debug(
            `[Messenger] webview message type: ${messageType}, data: ${msg}`,
          );
          try {
            ...
            console.error(
              `[Messenger] webview message type: ${messageType}, data: ${msg}, error: ${e}`,
            );
          }
        });
      });
      ...
      CORE_TO_WEBVIEW_PASS_THROUGH.forEach((messageType) => {
        this.inProcessMessenger.externalOn(messageType, async (msg) => {
          console.debug(
            `[Messenger] core message type: ${messageType}, data: ${msg}`,
          );
